...
module.exports = {
	...
	onTreeViewEntryClicked(e) {
		...
		// we got a file name
		if (this.active && e.detail != 1 && e.target.dataset.name != null) {
			// get the extension without the starting dot
			const ext = Path.extname(e.target.dataset.name).toLowerCase().replace(/^\./g,"");
			if (this.isUnsupported(ext)) {
				Shell.openExternal(e.target.dataset.path);
				return false;
			} else {
